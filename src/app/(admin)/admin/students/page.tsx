...
export default async function AdminStudents() {
  ...
  const { data: students } = await sb
    .from('profiles')
    .select('id,name,phone,phone_verified_at,city,is_minor,role,created_at')
    .eq('role','student')
    .is('deleted_at', null)
    .order('created_at', { ascending: false })

  // Get emails safely — service role key may not be set yet
  ...
  try {
    ...
    // SERVICE_ROLE_KEY not set yet — emails won't show, non-critical
  }
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        ...
        <p className="text-white/30 text-sm mt-0.5">{students?.length ?? 0} registered</p>
      </div>
      <div className="bg-white/3 border border-white/8 rounded-2xl overflow-hidden">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-white/5">
              {['Name','Email','Phone','City','Minor','Joined'].map(h => (
          ...
          <tbody className="divide-y divide-white/5">
            {students?.map(s => (
              <tr key={s.id} className="hover:bg-white/3 transition-colors">
                ...
                <td className="px-5 py-3.5">
                  {s.is_minor
                    ? <span className="text-[10px] bg-amber-500/15 text-amber-400 px-2 py-0.5 rounded-full">Minor</span>
            ...
            {!students?.length && (
              <tr><td colSpan={6} className="px-5 py-12 text-center text-white/20 text-sm">No students yet</td></tr>
            )}
          </tbody>
        </table>
      </div>
