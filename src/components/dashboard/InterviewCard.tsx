...
export default function InterviewCard({ application }: Props) {
  ...
  const calendarUrl = `https://calendar.google.com/calendar/r/eventedit?text=${encodeURIComponent(
    `Interview — ${application.college.name}`
  )}&dates=${date.toISOString().replace(/[-:]/g, '').split('.')[0]}Z/${new Date(
    date.getTime() + 60 * 60 * 1000
  )
    .toISOString()
    .replace(/[-:]/g, '')
    .split('.')[0]}Z&details=${encodeURIComponent(
    `College application interview for ${application.course.name}`
  )}`
    ...
    <div className="bg-purple-50 border border-purple-200 rounded-2xl p-5 mb-4">
      <div className="flex items-start gap-3 mb-4">
        ...
        <div>
          ...
          <p className="text-purple-700 text-xs mt-0.5">
            {application.college.name} has scheduled an interview for your application.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white border border-purple-100 rounded-xl p-4 space-y-3 mb-4">
        <div className="flex items-center gap-3 text-sm">
          ...
          <span className="text-heading font-semibold">{formattedDate}</span>
        </div>
        <div className="flex items-center gap-3 text-sm">
          <Clock className="w-4 h-4 text-purple-500 flex-shrink-0" />
          <span className="text-heading font-semibold">{formattedTime} IST</span>
        </div>
        <div className="flex items-center gap-3 text-sm text-body">
          <MapPin className="w-4 h-4 text-purple-500 flex-shrink-0" />
          <span>Contact the college for venue / video link details</span>
        </div>
      </div>

      {isPast ? (
        <div className="flex items-center gap-2 text-xs text-muted bg-slate-50 border border-border rounded-xl px-3 py-2">
          <CheckCircle className="w-3.5 h-3.5 text-success" />
          Interview date has passed. Check your application status for updates.
        </div>
      ) : (
        <div className="flex gap-2 flex-wrap">
          ...
          <button
            onClick={() => {
              navigator.clipboard.writeText(
                `Interview on ${formattedDate} at ${formattedTime} IST — ${application.college.name}`
          ...
          >
            {copied ? <CheckCircle className="w-3.5 h-3.5 text-success" /> : <Clock className="w-3.5 h-3.5" />}
